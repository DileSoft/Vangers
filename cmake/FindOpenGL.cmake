...
``OPENGL_gl_LIBRARY``
 ...
 targets.
``OPENGL_egl_LIBRARY``
 Path to the 'EGL' library. The ``OpenGL::EGL`` target should be preferred.
``OPENGL_gles2_LIBRARY``
 Path to the 'GLESv2' library. The ``OpenGL::GLES2`` target should be
 preferred.
``OPENGL_gles3_LIBRARY``
 Path to the 'GLESv3' library. The ``OpenGL::GLES3`` target should be
 preferred.
...
EGL targets are only provided if an appropriate backend is found, namely, one
of the ``OpenGL``, ``GLES2``, or ``GLES3`` components. Non-GLES support
requires GLVND to be present on the system.
...
if(OPENGL_FOUND)
  ...
  # ::EGL is provided if there is an implementation available and the library
  # has been found.
